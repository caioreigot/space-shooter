                    ...
                    for (let j = 0; j < enemySpaceship._enemys.length; j++) {
                        ...
                        if (
                            ...
                            if (enemyJ.lifes == 0) {
                                ...
                                enemySpaceship.lastEnemyAxis.x = enemyJ.x; enemySpaceship.lastEnemyAxis.y = enemyJ.y;
                                enemySpaceship.explosiumAnimation = true;
                        ...
                        if (
                            ...
                            && shot.y + shot.height >= meteorK.y
                               ...
                               if (meteorK.life == 0) {
                                    ...
                                    meteor.explosiumAnimation = true;
                                    meteor.lastMeteorAxis.x = meteorK.x;
                                    meteor.lastMeteorAxis.y = meteorK.y;
                               }
                           }
...
enemySpaceship = {
    ...
    _enemys: [],
    _spritesSmokes: [smoke1, smoke2],

    animationIndex: 0,
    animationSpeed: 3,
    maxAnimationSpeed: 6,
    explosiumAnimation: false,
    lastEnemyAxis: {x: null, y: null},
    ...
    draw() {
        if (spaceship.score < 75)
            this.currentEnemySprite = spriteAlienEnemySpaceship;
        else if (spaceship.score > 75 && spaceship.score < 150)
            this.currentEnemySprite = sprite2PhaseEnemySpaceship;
        else if (spaceship.score > 150 && spaceship.score < 225)
            this.currentEnemySprite = sprite3PhaseEnemySpaceship;
        else
            this.currentEnemySprite = sprite4PhaseEnemySpaceship;
        
        for (let i = 0; i < this._enemys.length; i++) {
            let enemy = this._enemys[i];

            this.currentEnemySprite.draw(enemy.x, enemy.y);
        }

        // Enemy explosium animation
        if (this.explosiumAnimation == true) {
            let currentSmoke = this._spritesSmokes[this.animationIndex];

            if (this.animationSpeed == 0) {
                if (this.animationIndex + 1 <= this._spritesSmokes.length)
                    this.animationIndex++;
                this.animationSpeed = this.maxAnimationSpeed;
            } else {
                this.animationSpeed--;
            }

            if (this.animationIndex == this._spritesSmokes.length) {
                this.animationIndex = 0;
                this.animationSpeed = this.maxAnimationSpeed;
                this.explosiumAnimation = false;
            } else {
                switch(this.animationIndex) {
                    case 0:
                        currentSmoke.draw(this.lastEnemyAxis.x, this.lastEnemyAxis.y);
                        break;
                    case 1:
                        currentSmoke.draw(this.lastEnemyAxis.x - 5, this.lastEnemyAxis.y - 8);
                        break;
                    default:
                        break;
                }
...
meteor = {
    ...
    _spritesSmokes: [smoke1, smoke2, smoke3, smoke4, smoke5],
    ...
    update() {
        ...
        for (let i = 0; i < this._meteors.length; i++) {
            ...
            if (meteor.y > canvas.height) {
                ...
                this.lastMeteorAxis.x = meteor.x; this.lastMeteorAxis.y = meteor.y;
                this.explosiumAnimation = true;
            }
    ...
    draw() {
        ...
        if (this.explosiumAnimation == true) {
            ...
            if (this.animationIndex == this._spritesSmokes.length) {
                ...
                switch (this.animationIndex) {
                    case 0:
                        currentSmoke.draw(this.lastMeteorAxis.x, this.lastMeteorAxis.y);
                        break;
                    case 1:
                        currentSmoke.draw(this.lastMeteorAxis.x - 5, this.lastMeteorAxis.y - 15);
                        break;
                    case 2:
                        currentSmoke.draw(this.lastMeteorAxis.x - 15, this.lastMeteorAxis.y - 25);
                        break;
                    case 3:
                    case 4:
                        currentSmoke.draw(this.lastMeteorAxis.x - 25, this.lastMeteorAxis.y - 35);
                        break;
                    default:
                        break;
                }
...
function update() {
    ...
    if (bossPhase == true) {
      ...
      if (currentState == states.lose) {
            ...
            bossPhase = false;
            meteor.explosiumAnimation = false;
            enemySpaceship.explosiumAnimation = false;
